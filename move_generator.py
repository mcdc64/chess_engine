...
def get_piece_moves(board,i,j,piece_type):
    pseudo_legal_moves = []
...
def get_attacked_squares(board,attacking_color): # get a bit board showing squares attacked by a certain color
    ...
    if(board.color_to_move != attacking_color):
        board.toggle_color()
        switched = True
    for position in attacking_pieces:
        piece = board.pieces[position[0]][position[1]]
        moves = get_piece_moves(board,position[0],position[1],piece.piece_type)

        attacked_squares += [move.new_position for move in moves]
        print("Squares: "+str([move.new_position for move in moves])+" "+str(attacking_color))
        print(piece.piece_type)
    for square in attacked_squares:
        bit_board[attacked_squares[0],attacked_squares[1]] = True
    if(switched):
        board.toggle_color()
    return bit_board
